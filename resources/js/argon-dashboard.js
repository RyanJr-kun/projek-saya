...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function sidebarType(a) {
        ...
        if (color == "bg-transparent" && bodyDark) {
            var textDarks = document.querySelectorAll(
        ...
        if (window.innerWidth < 991) {
            total.forEach(function (item, i) {
                if (!item.classList.contains("flex-column")) {
                    ...
                    let li =
                        item.querySelector(".nav-link.active").parentElement;
                    let nodes = Array.from(li.closest("ul").children); // get array
                    let index = nodes.indexOf(li) + 1;
                    let sum = 0;
                    for (var j = 1; j <= nodes.indexOf(li); j++) {
                    ...
                    moving_div.style.width =
                        item.querySelector("li:nth-child(1)").offsetWidth +
                        "px";
                    moving_div.style.transform =
                        "translate3d(0px," + sum + "px, 0px)";
                }
    ...
    function darkMode(el) {
        ...
        const text_strong_white =
            document.querySelectorAll("strong.text-white");
